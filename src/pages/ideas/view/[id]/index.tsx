...
import { deleteCollaborationById } from 'apiSdk/collaborations';
...
function IdeaViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<IdeaInterface>(
    ...
    () =>
      getIdeaById(id, {
        relations: ['user', 'collaboration', 'feedback', 'task'],
      }),
  );

  const collaborationHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('idea', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/ideas/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('collaboration', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/collaborations/create?idea_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.collaboration?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/collaborations/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('collaboration', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/collaborations/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('collaboration', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    collaborationHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/feedbacks/create?idea_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
              ...
              {hasAccess('task', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/tasks/create?idea_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
